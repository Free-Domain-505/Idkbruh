...
def handle_requests():
    ...
    def process_request():
        ...
        if not before:
            return {"error": "Server ne data nahi diya, token ya version galat hai."}, 500
            
        jsone = MessageToJson(before)
        data = json.loads(jsone)
        
        # Crash se bachne ke liye check karein
        if 'AccountInfo' not in data:
            return {"error": "AccountInfo nahi mila. Token expired ho sakta hai."}, 500
